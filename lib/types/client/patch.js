 ...
 */
const proto = LocaleRuntime.prototype;
 ...
 */
export function probeLocaleRuntime() {
 ...
 */
export function installPatch(store) {
    if (origLookup === undefined) {
        console.error('[better-locale] LocaleRuntime.prototype.lookup not found; upstream changed shape. '
            + 'Patch skipped; override service is inert. Pin a compatible dsh-client-locale version.');
        return () => { };
    }
    if (proto.lookup !== origLookup) {
        console.error('[better-locale] LocaleRuntime.prototype.lookup already patched; '
    ...
    return () => {
        if (proto.lookup !== wrapper)
            return; // someone else replaced it; leave alone
 ...
 */
export function bumpRevision(locale) {

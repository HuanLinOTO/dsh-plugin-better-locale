...
export class BetterLocaleStore {
    ...
    registerLanguage(def) {
        ...
        this.notify();
        return () => {
            if (!this._languages.some(l => l.id === def.id))
                return;
     ...
     */
    setActive(localeId) {
        if (this.activeOverride === localeId)
            return;
 ...
 */
export function loadActiveFromStorage() {
    try {
        if (typeof localStorage === 'undefined')
            return undefined;
